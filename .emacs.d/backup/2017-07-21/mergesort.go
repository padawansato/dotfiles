...
// UNIX 時間をシードにして乱数生成器を用意する
    ...
    // ソートして結果を出力する
		...
		log.Print("end")    fmt.Println(runtime.NumCPU())
    fmt.Println(runtime.NumCPU()) //利用できる CPU のコア数
    fmt.Println(runtime.GOMAXPROCS(0)) //使用するコア数
    fmt.Println(runtime.NumGoroutine()) //現存している goroutine の数
    }
